 ...
 * @LastEditors: 南桥几许
 * @LastEditTime: 2022-07-19 11:54:33
 * @FilePath: \organization\src\views\onlineSchool\know\class\courseExam\addExamBao.vue
-->
<template>
  <BasicModal
    v-bind="$attrs"
  ...
  export default defineComponent({
    ...
    setup(_, { emit }) {
      ...
      const [registerTable, { reload }] = useTable({
        title: '',
        api: examClient.getQuestionPackageList,
        beforeFetch: (t: any) => {
          t.kindId = unref(kind);
          t.majorId = unref(zyId);
        },
        fetchSetting: {
          // 传递到后台的当前页面的字段名
          pageField: 'pageNum',
         ...
         * slots：自定义的插槽 */
        columns: [
          { title: '试卷包名称', dataIndex: 'name', align: 'left' },
          { title: '项目分类', dataIndex: 'kindName' },
          { title: '所属专业', dataIndex: 'majorName' },
        ],
