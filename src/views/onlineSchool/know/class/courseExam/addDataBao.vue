 ...
 * @LastEditors: 南桥几许
 * @LastEditTime: 2022-07-19 12:01:24
 * @FilePath: \organization\src\views\onlineSchool\know\class\courseExam\addDataBao.vue
-->
<template>
  <BasicModal
    v-bind="$attrs"
  ...
  export default defineComponent({
    ...
    setup(_, { emit }) {
      ...
      const [registerTable, { reload }] = useTable({
        title: '',
        api: examClient.getDocPackagePage,
        beforeFetch: (t: any) => {
          t.isPass = 1;
          t.kind = unref(kind);
          t.zyId = unref(zyId);
      ...
      function getCheckboxProps(record) {
        if (unref(ids) && unref(ids).length > 0) {
          return {
