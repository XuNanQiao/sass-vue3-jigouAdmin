 ...
 * @LastEditors: 南桥几许
 * @LastEditTime: 2022-07-07 17:32:37
 * @FilePath: \organization\src\views\onlineSchool\know\course\courseExam\addExam.vue
-->
<template>
  <BasicModal
    v-bind="$attrs"
  ...
  export default defineComponent({
    ...
    setup(_, { emit }) {
      ...
      const [registerTable, { reload }] = useTable({
        title: '',
        api: examClient.getQuestionPaperList,
        beforeFetch: (t: any) => {
          t.kindId = unref(kind);
        },
        fetchSetting: {
         ...
         * slots：自定义的插槽 */
        columns: [
          { title: '试卷名称', dataIndex: 'name', align: 'left' },
          { title: '项目分类', dataIndex: 'kindName' },
          { title: '所属课程', dataIndex: 'courseName' },
          { title: '题数', dataIndex: 'subjectNum' },
