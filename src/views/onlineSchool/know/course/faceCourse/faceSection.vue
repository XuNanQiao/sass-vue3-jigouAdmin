 ...
 * @LastEditTime: 2022-07-10 14:31:43
 * @LastEditors: 南桥几许
 * @Description: 
 * @FilePath: \organization\src\views\onlineSchool\know\course\faceCourse\faceSection.vue
 * 
-->
<template>
  <div>
    <BasicTable @register="registerTable">
      <template #toolbar>
        <a-button type="primary" @click="handleCreate"> 新增授课大纲</a-button>
      </template>
...
<script lang="ts" setup>
  ...
  const [registerTable, { reload }] = useTable({
    title: '',
    api: courseClient.getFaceTeachOutlineList,
    beforeFetch: (t: any) => {
     ...
     * slots：自定义的插槽 */
    columns: [
      { title: '授课大纲名称', dataIndex: 'name' },
      { title: '上课时间', dataIndex: 'attendTime' },
  ...
  function handleDelete(record: Recordable) {
    courseClient.deleteFaceTeachOutline(record.id).then((res) => {
      reload();
    });
  }
  ...
  function handleDoc(record: Recordable, docType: number) {
    router.push(`/know/course/courseData?typeId=${record.id}&type=5&docType=${docType}`);
  }
</script>
