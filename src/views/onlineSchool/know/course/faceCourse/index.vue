 ...
 * @LastEditTime: 2022-07-07 11:39:59
 * @LastEditors: 南桥几许
 * @Description: 
 * @FilePath: \organization\src\views\onlineSchool\know\course\faceCourse\index.vue
 * 
-->
<template>
  <div>
    ...
    <BasicTable @register="registerTable">
      ...
      <template #name="{ record }">
        <div class="tabNameImg">
          <img class="rowImg" :src="record.pcimg" alt="" />
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const [registerTable, { reload }] = useTable({
        title: '',
        api: courseClient.getFaceTeachList,
        beforeFetch: (t: any) => {
         ...
         * slots：自定义的插槽 */
        columns: [
          { title: '面授课名称', dataIndex: 'name', slots: { customRender: 'name' } },
          ...
          { title: '上课地点', dataIndex: 'attendPlace', align: 'left' },
      ...
      function handleDelete(record: Recordable) {
        courseClient.deleteFaceTeach(record.id).then((res) => {
          reload();
        });
      }
      /* 章节管理 */
      ...
      function handleEvaluate(record: Recordable) {
        router.push(`/know/score/stuComment?typeId=${record.id}&type=10`);
      }
      /* 课程资料 */
      function handleData(record: Recordable) {
        router.push(`/know/course/courseData?typeId=${record.id}&type=5&docType=3`);
      }
      /* 课程模考 */
      ...
      function showChange(record: Recordable) {
        courseClient.updateFaceTeachState(record.id).then((res) => {
          reload();
        });
      }
      return {
