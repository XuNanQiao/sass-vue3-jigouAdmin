 ...
 * @LastEditTime: 2022-07-07 17:40:11
 * @LastEditors: 南桥几许
 * @Description: 
 * @FilePath: \organization\src\views\onlineSchool\know\course\audio\index.vue
 * 
-->
<template>
  <div>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const [registerTable, { reload }] = useTable({
        title: '',
        api: courseClient.getAudioTeachList,
        beforeFetch: (t: any) => {
      ...
      function handleDelete(record: Recordable) {
        courseClient.deleteAudioTeach(record.id).then((res) => {
          reload();
        });
      }
      /* 章节管理 */
      ...
      function handleEvaluate(record: Recordable) {
        router.push(`/know/score/stuComment?typeId=${record.id}&type=7`);
      }
      /* 课程资料 */
      function handleData(record: Recordable) {
        router.push(`/know/course/courseData?typeId=${record.id}&type=3&docType=3`);
      }
      /* 课程模考 */
      ...
      function showChange(record: Recordable) {
        courseClient.updateAudioTeachState(record.id).then((res) => {
          reload();
        });
      }
      return {
