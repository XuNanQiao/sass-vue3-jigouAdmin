<template>
  <div>
    ...
    <BasicTable @register="registerTable">
      <template #toolbar>
        <a-button type="primary" @click="handleCreate"> 新增考点</a-button>
      </template>
      <template #showStatus="{ record }">
        <!-- {{ record }} -->
        <a-switch
          @click="switchChange(record)"
          :checked="record.showStatus == 1 ? false : true"
          checked-children="开"
    ...
    <testCenterDrawer @register="registerDrawer" @success="handleSuccess" />
  </div>
</template>
<script lang="ts">
  ...
  import testCenterDrawer from './testCenterDrawer.vue';
  import { Modal } from 'ant-design-vue';
  export default defineComponent({
    name: 'RoleManagement',
    components: { BasicTable, TableAction, KindSearch, testCenterDrawer },
    setup() {
      ...
      const [registerTable, { reload }] =  useTable({
        ...
        title: '',
        api: testCentera.getExamPlaceList,
        beforeFetch: (t: any) => {
      ...
      function handleDelete(record: Recordable) {
        Modal.confirm({
          ...
          cancelText: '取消',
          async onOk() {
            await testCentera.deleteExamPlace({ id: record.id });
            reload();
          },
        });
      }
      ...
      async function switchChange(record: any) {
        await testCentera.updateShowStatus({ id: record.id });
        reload();
      }
      return {
        ...state,
