 ...
 * @LastEditors: 隐世浮华
 * @LastEditTime: 2022-07-07 11:31:46
 * @FilePath: \edub_tob_jxyZ\src\views\onlineSchool\exam\exambag\exambagDrawer.vue
-->
<template>
  <BasicDrawer
    v-bind="$attrs"
    ...
    setup(_, { emit }) {
      ...
      async function handleSubmit() {
        try {
          ...
          if (!unref(isUpdate)) {
            // 添加入驻商模板
            await exambaga.addQuestionPackage(values);
          } else {
            // 修改入驻商模板
            // values.roomId = unref(datas).roomId; (未改)
            await exambaga.updateQuestionPackage(values);
          }
          closeDrawer();
          emit('success');
        } finally {
